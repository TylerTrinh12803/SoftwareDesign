...
const Notification = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      const storedUserId = localStorage.getItem("userId"); // Ensure user_id is correctly retrieved
      if (!storedUserId) {
      ...
      try {
        const response = await fetch(`http://localhost:5000/notifications?volunteer_id=${storedUserId}`);
        if (!response.ok) {
  ...
  const dismissAllNotifications = async () => {
    try {
      await fetch('http://localhost:5000/api/notifications/dismiss-all', {
        method: 'DELETE',
      });
      setNotifications([]);
    } catch (error) {
